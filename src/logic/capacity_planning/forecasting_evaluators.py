...
# class Evaluators:
#     
#     def evaluateTarget(self, requestContext, target):
#         tokens = grammar.parseString(target)
#         result = self.evaluateTokens(requestContext, tokens)
#     
...
# 
# 
#     def evaluateTokens(self, requestContext, tokens, replacements=None):
#         
#         if tokens.template:
#             arglist = dict()
#             if tokens.template.kwargs:
#                 arglist.update(dict([(kwarg.argname, self.evaluateTokens(requestContext, kwarg.args[0])) for kwarg in tokens.template.kwargs]))
#             if tokens.template.args:
#                 arglist.update(dict([(str(i+1), self.evaluateTokens(requestContext, arg)) for i, arg in enumerate(tokens.template.args)]))
#             if 'template' in requestContext:
#                 arglist.update(requestContext['template'])
#             return self.evaluateTokens(requestContext, tokens.template, arglist)
#         
#         elif tokens.expression:
#             return self.evaluateTokens(requestContext, tokens.expression, replacements)
#         
...
#             args = [self.evaluateTokens(requestContext, arg, replacements) for arg in tokens.call.args]
#             requestContext['args'] = tokens.call.args
#             kwargs = dict([(kwarg.argname, self.evaluateTokens(requestContext, kwarg.args[0], replacements))
