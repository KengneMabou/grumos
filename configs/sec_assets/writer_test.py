...
db = connect('grumos', username='',password='',host='172.16.0.124')
...
class Incident(Document):
	...
	machines_impacted = ListField(StringField(max_length=250))#the first
	incident_type = StringField(verbose_name="Incident severety",choices = incident_type_choices)
	...
	resolution_date = DateTimeField(default = datetime.datetime.now()) # will be use to process MTTR
	...
	occurences = IntField(verbose_name="Number of occurrences", default=1) # don't delete
...
def create_incident(arguments):
	...
	inc = Incident(detected_by="correlation",incident_recorder="SEC (Simple Event Correlator)", state="open", description=arguments["description"], gravity=arguments["gravity"],incident_type=arguments["incident_type"],name=arguments["name"]).save()
